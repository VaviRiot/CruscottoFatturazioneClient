...
export class NotificationsComponent implements OnInit {
  ...
  public notificheListOverview = new NotificheListOverview(0, null);
  ...
  listFilters: Array<Array<string>> = [
    ['nome', 'lke'],
    ['tipologiaNotifica', 'lke'],
    ['workflowStepNotifiche', 'lke'],
    ['business', 'lke'],
    ['lastModDate', 'lke']
  ]

  listInFilters: string[] = ['nome'];
  
  /* SERVER SIDE FILTERS*/ 
  nomeFilter: string;
  tipologiaNotificaFilter: string;
  workflowStepNotificheFilter: string;
  ...
  @ViewChild("PCNT") table: ElementRef;
  ...
  ngOnInit(): void {
    ...
    this.dataSource = new DataSource({
      ...
      load: (loadOptions) => {
        ...
        return this.notificheService
          .getNotificheDataTable(authToken, filterPost)
          .toPromise()
          .then((res) => {

              res.lines.forEach(notificaApp => {

                if(notificaApp.lastModDate)
                {
                  notificaApp.lastModString = pipe.transform(notificaApp.lastModDate, 'dd/MM/yyyy HH:mm');
                }
              });
              
              this.notificheListOverview = res;
              // console.log(this.notificheListOverview);
    ...
    modalDialog.afterClosed().subscribe(res => 
    {
      if (res == true)
      {
        ...
        this.deleteSubscription = this.notificheService.deleteNotifica(auth_token, notification_id, this.userLogged.name).subscribe((res: boolean) => 
        {
          ...
          {
            //console.log(res);
            this.common.sendUpdate("showAlertInfo", "Notifica rimossa correttamente!");
  ...
  tableStateLoad = () => {
    ...
    localStorage.setItem(environment.gmtNotListFilter, JSON.stringify(""))
    //console.log(localStorage.getItem(environment.gmtNotListFilter));
    var data = JSON.parse(
      localStorage.getItem(environment.gmtNotListFilter)
    )
    return data;
  }
