...
export class RolesComponent implements OnInit {
  ...
  public ruoliListOverview = new RuoliListOverview(0, null);
  ...
  @ViewChild("PCRL") table: ElementRef;
    ...
    this.dataSource = new DataSource({
      ...
      load: (loadOptions) => {
        ...
        return this.userService
          .getRuoliDataTable(authToken, filterPost)
          .toPromise()
          .then((res) => {

            res.lines.forEach(roleApp => 
            {
              if(roleApp.lastModDate)
              {
                roleApp.lastModString = pipe.transform(roleApp.lastModDate, 'dd/MM/yyyy HH:mm');
              }
            });

              this.ruoliListOverview = res;
  ...
  tableStateLoad = () => {
    var data = JSON.parse(
      localStorage.getItem(environment.gmtRoleListFilter),
    )
    return data
  }
  ...
  deleteRole(role_id: number)
  {
    ...
    modalDialog.componentInstance.messageText = "Sei sicuro di voler cancellare il ruolo?";
    ...
    modalDialog.afterClosed().subscribe(res => 
    {
      if (res == true)
      {
        ...
        this.deleteSubscription = this.rolesService.deleteRole(auth_token, role_id, this.userLogged.name).subscribe((res: boolean) => 
        {
          if(res)
          {
            //console.log(res);
            this.common.sendUpdate("showAlertInfo", "Ruolo rimosso correttamente!");
          ...
          {
            this.common.sendUpdate("hideSpinner");
            this.common.sendUpdate("showAlertDanger", "Impossibile rimuovere il ruolo al momento.");
          }
        },
        error => {
